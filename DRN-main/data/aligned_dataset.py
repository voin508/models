...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        def normalize_percentile(img, low_q=0.005, high_q=0.995):
            out = []
            for c in range(img.shape[0]):
                channel = img[c]
                low = torch.quantile(channel, low_q)
                high = torch.quantile(channel, high_q)
                channel = (channel - low) / (high - low + 1e-6)
                out.append(channel.clamp(0,1))
        ...
        if not self.opt.no_flip and  np.random.uniform() < 0.5:
            A = A[:, ::-1, :]
            B = B[:, ::-1, :]
